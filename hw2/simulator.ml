...
let smallest (x:int64) : bool = x = Int64.min_int
...
let step (m:mach) : unit =
  ...
  match operator,(unifier location m) with
    ...
    | Subq, [Ind1 (Lit x); Reg y] -> let n = int64_of_sbytes (frommem m x) in let z = m.regs.(rind y) in let result = Int64_overflow.sub z n in (m.regs.(rind y) <- result.Int64_overflow.value; setter m [result.Int64_overflow.overflow || smallest n; result.Int64_overflow.value < 0L; result.Int64_overflow.value = 0L])
    | Subq, [Reg x; Ind1 (Lit y)] -> let n = int64_of_sbytes (frommem m y) in let z = m.regs.(rind x) in let result = Int64_overflow.sub n z in (tomem m y (sbytes_of_int64 result.Int64_overflow.value); setter m [result.Int64_overflow.overflow || smallest z; result.Int64_overflow.value < 0L; result.Int64_overflow.value = 0L])
    | Subq, [Reg x; Reg y] -> let n = m.regs.(rind y) in let z = m.regs.(rind x) in let result = Int64_overflow.sub n z in (m.regs.(rind y) <- result.Int64_overflow.value; setter m [result.Int64_overflow.overflow || smallest z; result.Int64_overflow.value < 0L; result.Int64_overflow.value = 0L])
    | Subq, [Imm (Lit z); Reg y] -> let n = m.regs.(rind y) in let result = Int64_overflow.sub n z in (m.regs.(rind y) <- result.Int64_overflow.value; setter m [result.Int64_overflow.overflow || smallest z; result.Int64_overflow.value < 0L; result.Int64_overflow.value = 0L])
    | Subq, [Imm (Lit z); Ind1 (Lit y)] -> let n = int64_of_sbytes (frommem m y) in let result = Int64_overflow.sub n z in (tomem m y (sbytes_of_int64 result.Int64_overflow.value); setter m [result.Int64_overflow.overflow || smallest z; result.Int64_overflow.value < 0L; result.Int64_overflow.value = 0L])
