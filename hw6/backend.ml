...
type edge = uid * uid
>>>>>>> a8868c614a98f05907b3ea36fcfe33ad034e84d6
  ...
  let rec helper graph list i = 
    ...
    | Some c -> 
      ...
      | x::xs -> 
        ...
        | Id id -> helper (UidM.update (fun (a, b) -> (a,(Alloc.LReg c)::b)) id graph) xs (i+1)
  ...
  let add_edges (graph:graph) (edges_list:edge list) = 
    List.fold_left (
      fun (graph:graph) (a, b: uid * uid) -> (
        UidM.update (
          fun (neighbors, locs: UidS.t * Alloc.loc list) -> 
            (UidS.add b neighbors), locs
        ) a graph
      )
    ) graph edges_list 
  in
  let rec allocate_func_args (graph:graph) (op_list:(ty * Ll.operand) list) (i:int) =
    ...
    | Some l ->
      ...
      | (_, op)::ops ->
        begin match op with
        | Id uid -> 
          let new_graph = (UidM.update (
              fun (neighbors, locs: UidS.t * Alloc.loc list) : (UidS.t * Alloc.loc list) -> 
                (neighbors, (Alloc.LReg l)::locs)
            ) uid graph)
          in
          allocate_func_args new_graph ops (i + 1)
        | _ -> allocate_func_args graph ops (i + 1)
        end
      end
    | None ->  graph
  in
  let prepare_call (graph:graph) (insns : insn) = 
    match insns with
    | Call (_, _, args_list) -> allocate_func_args graph args_list 0
    | _ -> graph
  in
  ...
  let add_block_edges (graph:graph) (block:block) (locs : (uid * Alloc.loc) list) =
    ...
    let new_graph = List.fold_left (
      fun (graph:graph) (uid, ins : uid * insn) -> 
        let new_graph = (add_edges graph (get_new_edges (live.live_in uid))) in
        prepare_call new_graph ins
    ) graph block.insns in
    let term_uid = fst block.term in
    let new_graph_term = add_edges new_graph (get_new_edges (live.live_in term_uid)) in
